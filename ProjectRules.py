...
4. No individuals or families are never added purely from the GEDCOM; they must map to an
    existing census golden record.
5. Auxiliary data such as names, marriage information, additional residences, military services,
    and deaths are added from information from the GEDCOM and other sources.
...
7. All requirements are to be tested via the logging data and analytic routines written to read
 ...
 8. Andy's job is to provide the logging data and JSON data so that the AI can have access to it.
 9. Gemini Code Assist's job is to write the code and analyze the logging data
     Then to make changes to the code based on logs that show errors in the processing.
...
Design:
    ...
    Script gedcom_analysis.py is used. 

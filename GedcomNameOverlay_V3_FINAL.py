...
YEAR_WINDOW = 1  # Tightened to +/- 1 year to reduce noise!
IGNORE_KIDS_FOR_VALIDATION = False  # Set to False to activate the Kid Fingerprint filter!
VERBOSE_LOGGING = False  # Set to False to silence detailed evaluations and only show the final summary!
...
def run_overlay_v3(logger):
    ...
    # Here we connect to DuckDB, set up our memory limits, and load all the
    # translation dictionaries (like turning IPUMS state codes into real names)
    # so Python knows how to read the census data.
    # =========================================================================
    ...
    # -------------------------------------------------------------------------
    # JSON PREPARATION: The "Elite Target" Filter
    # -------------------------------------------------------------------------
    # We don't want to waste time searching the database for ancestors who are
    # missing birthplaces. This block loops through your Ancestry GEDCOM data
    # and ONLY keeps the couples who have a complete demographic fingerprint.
    ...
    con.execute(
        "CREATE TABLE targets (idx INTEGER, h_byr INTEGER, h_bpl INTEGER, h_fbpl INTEGER, h_mbpl INTEGER, w_byr INTEGER, w_bpl INTEGER, w_fbpl INTEGER, w_mbpl INTEGER, kid_fingerprint INTEGER, h_first VARCHAR, h_last VARCHAR, w_first VARCHAR, w_last VARCHAR)")
    con.executemany("INSERT INTO targets VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", target_rows)

    kid_match_condition_sql = ""
    if not IGNORE_KIDS_FOR_VALIDATION:
        kid_match_condition_sql = "AND (t.kid_fingerprint = 0 OR f.kids_byr_sum = t.kid_fingerprint)"

    all_matches = []

    # -------------------------------------------------------------------------
    # SQL FILTER SETUP: Building the Query Rules
    # -------------------------------------------------------------------------
    # These strings get injected into our massive DuckDB query below.
    # We dynamically build the exact rules DuckDB has to follow so we don't
    ...
    # =========================================================================
    # PHASE 1: ONE-SHOT DEMOGRAPHIC CROSS-REFERENCE (THE HAYSTACK)
    # =========================================================================
    # What this does: It asks DuckDB to search all historical records in the
    #                 Time Machine in a single query, looking for couples who
    #                 perfectly match the ages and birthplaces of our Targets.
    # Why we do it:   We need to gather all the "demographic clones" (people who
    #                 look exactly like your ancestors on paper) so we can
    #                 scrutinize their migration paths in Phase 2.
    # =========================================================================
    logger.info(f"\n=======================================================")
    logger.info(f"PHASE 1: ONE-SHOT DEMOGRAPHIC CROSS-REFERENCE (ALL DECADES)")
    logger.info(f"=======================================================")

    # Step 1A: Grab just the Heads and Spouses (ignoring children for now to save memory)
    con.execute(
    ...
    con.execute(f"""
        ...
        WITH parsed_inds AS (
            SELECT i.*,
                   -- Step 1B: Clean up the raw IPUMS data so it is easy to search.
                   -- Specifically, we take the 4-digit birthplaces (like 4200) and chop them 
                   -- down to 2 digits (like 42) so they match our Ancestry JSON targets.
    ...
    # Step 1C: THE MASSIVE QUERY.
    # This joins the target JSON rules to the cleaned database.
    # It mathematically enforces the Husband's Age/BPL, the Wife's Age/BPL,
    # all 4 parents' BPLs, and the Kid Fingerprint (if activated).
    ...
    if all_matches:
        # =========================================================================
        # PHASE 2: EVALUATING MULTI-DECADE TRAJECTORIES (THE NEEDLE)
        # =========================================================================
        # What this does: It takes the clones we found in Phase 1 and checks which
        #                 counties they lived in over the decades (e.g., 1880, 1900).
        #                 It compares their database counties against the counties
        #                 you typed into your Ancestry tree (from the JSON).
        # Why we do it:   Even if there are 4,600 clones born in PA in 1880, it is
        #                 mathematically impossible for an imposter to move to the
        #                 exact same random sequence of counties as your ancestor.
        #                 This phase eliminates the 4,599 imposters!
        # =========================================================================
        logger.info(f"\n=======================================================")
        ...
        grouped_matches = {}
        # Step 2A: Group the thousands of random matches by "Clan ID".
        # A Clan is just a single family tracked across multiple decades.
        ...
        for t_row in target_rows:
            ...
            if target_idx not in grouped_matches:
                if VERBOSE_LOGGING:
                    logger.info("   [-] REJECTED: 0 demographic candidates found in Time Machine.")
                continue

            clans = grouped_matches[target_idx]
            total_cands = sum(len(c) for c in clans.values())
            if VERBOSE_LOGGING:
                logger.info(
                    f"   -> Found {total_cands} total demographic candidate records across {len(clans)} Clan(s).")
            ...
            for clan_id, clan_matches in clans.items():
                ...
                for cand in clan_matches:
                    ...
                    # ---------------------------------------------------------
                    # THE GEO CHECK (Trajectory Math)
                    # Did this family live in the county our Ancestry tree says
                    # they were supposed to live in during this decade?
                    # ---------------------------------------------------------
                    ...
                    if cand[30] is not None:
                        try:
                            db_county_code = str(int(float(cand[30])))
                        except ValueError:
                            db_county_code = str(cand[30]).strip()

                    db_state_name = STATEICP_MAP.get(int(float(cand[31])), "Unknown State") if cand[
                                                                                                   31] is not None else "Unknown State"
                    ...
                    if geo_hit:
                        ...
                        traj_log.append(
                            f"  [{cand_year}] [-] DB: {db_county_name}, {db_state_name} | GEDCOM: {h_res_disp}")

                # ---------------------------------------------------------
                # THE FINAL SCORE
                # A Trajectory Score of 2 means the Clan matched the exact
                # Ancestry county in at least 2 different census decades.
                # ---------------------------------------------------------
                ...
                if is_trajectory_match or exact_hw_match:
                    for cand in clan_matches:
                        names_to_write.add((cand[32], h_first, h_last))
                        names_to_write.add((cand[33], w_first, w_last))

                if VERBOSE_LOGGING:
                    # Log the outcome for every evaluated clan to provide a complete diagnostic report
                    logger.info(
                        f" > CLAN: {clan_id} | Decades Found: {len(clan_matches)} | Trajectory Score: {trajectory_score}")
        ...
        # =========================================================================
        # PHASE 3: SAVING RESOLVED NAMES TO TIME MACHINE (THE SAVE BUTTON)
        # =========================================================================
        # What this does: Now that we found the 100% proven ancestors, we take their
        #                 blank database IDs and permanently attach their real names
        #                 (like "Foster Edgar Askey") to them in a final table.
        # Why we do it:   So the final GEDCOM exporter knows exactly who is who!
        # =========================================================================
        logger.info(f"\n=======================================================")
        ...
        # Step 3A: DuckDB hates writing rows one-by-one using Python logic.
        # To keep it lightning fast, we dump the real names into a temporary
        # CSV file on your hard drive...
        temp_csv = os.path.join(BASE_DATA_DIR, "duckdb_tmp", "temp_resolved_names.csv").replace('\\', '/')
        with open(temp_csv, 'w', newline='', encoding='utf-8') as f:
            csv.writer(f).writerows(list(names_to_write))

        # Step 3B: ...and then we tell DuckDB to 'COPY' that entire CSV into
        # the database table in 0.1 seconds!
        con.execute(f"COPY match_db.resolved_names FROM '{temp_csv}' (FORMAT CSV);")
        if os.path.exists(temp_csv): os.remove(temp_csv)

        logger.info(
            f"  -> Successfully painted {len(names_to_write):,} real ancestor records into DemographicMatches2.db!")
        logger.info(f"=======================================================\n")

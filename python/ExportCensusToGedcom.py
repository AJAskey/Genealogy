...
# Dictionary to translate IPUMS numeric codes back into English for the GEDCOM
REVERSE_BPL = {
...
def decode_bpld(bpld_str):
    ...
    try:
        val = int(bpld_str)
        # IPUMS detailed codes are usually 4 or 5 digits. Divide by 100 to get the prefix.
...
def export_gedcom(logger):
    ...
    with sqlite3.connect(NAMED_DB) as conn:
        ...
        cursor.execute(f"""
            ...
            FROM individuals 
            WHERE last_name COLLATE NOCASE = '{TARGET_LAST_NAME}'
        ...
        cursor.execute(f"""
            SELECT histid, first_name, last_name, sex, birthyr, bpld, family_id, father_histid, mother_histid 
            FROM individuals 

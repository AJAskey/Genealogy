...
def link_households_across_decades(logger):
    ...
    con.execute("""
        CREATE TEMP TABLE clean_ptrs AS 
        SELECT DISTINCT UNNEST(string_split(vault_pointers, '|')) AS comp_id FROM clean.golden_records;
    """)
    ...
    con.execute("""
        ...
        WHERE age IS NOT NULL AND bpld IS NOT NULL
          AND (related IN ('0100', '0200', '100', '200', 'Head', 'Spouse', 'Wife') 
               OR related ILIKE '%Head%' OR related ILIKE '%Spouse%' OR related ILIKE '%Wife%');
    """)
    
    logger.info("Step 3/5: Pairing up Households (Head + Spouse)...")
    con.execute("""
        CREATE TEMP TABLE hh_all AS
        SELECT 
        ...
        JOIN pop s ON h.serial = s.serial AND h.year = s.year 
            AND (s.related IN ('0200', '200', 'Spouse', 'Wife') OR s.related ILIKE '%Spouse%' OR s.related ILIKE '%Wife%')
    ...
    con.execute("""
        CREATE TEMP TABLE anchored_hh AS
        SELECT *, 
        ...
        FROM hh_all WHERE head_id IN (SELECT comp_id FROM clean_ptrs)
        UNION
        SELECT *, 
               CAST(year + 10 AS INTEGER) AS target_year,
               head_age + 8 AS min_head_age,
               ...
               spouse_age + 8 AS min_spouse_age,
               spouse_age + 12 AS max_spouse_age
        FROM hh_all WHERE spouse_id IN (SELECT comp_id FROM clean_ptrs);
    """)

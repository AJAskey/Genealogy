...
"""
-----------------------------------
File: _get_nfathers.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for NFATHERS codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for NFATHERS) is detected
...
_json_path = r"E:/Claude/JSON/nfathers_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("NFATHERS", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- NFATHERS lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load NFATHERS codes: {_e}")


# ---------------------------------------------------------------------------
# 2. GETTER
# ---------------------------------------------------------------------------
def get_nfathers(code):
    """
    Returns the description for a NFATHERS code.
...
if __name__ == "__main__":
    print(f"--- Testing NFATHERS Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_nfathers(t)
            print(f"      get_nfathers({t!r:>8}) -> {result}")
    else:

...
"""
-----------------------------------
File: _get_countyicp.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for COUNTYICP codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for COUNTYICP) is detected
...
_json_path = r"E:/Claude/JSON/countyicp_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("COUNTYICP", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- COUNTYICP lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load COUNTYICP codes: {_e}")


# ---------------------------------------------------------------------------
# 2. GETTER
# ---------------------------------------------------------------------------
def get_countyicp(code):
    """
    Returns the description for a COUNTYICP code.
...
if __name__ == "__main__":
    print(f"--- Testing COUNTYICP Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_countyicp(t)
            print(f"      get_countyicp({t!r:>8}) -> {result}")
    else:

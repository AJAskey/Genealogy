...
def build_test_vault(year):
    ...
    con.execute( \
        "CREATE TABLE test.families AS SELECT * FROM source.families WHERE head_histid IS NOT NULL AND spouse_histid IS NOT NULL AND num_kids > 0;")

    print("  -> Extracting relevant individuals...")
    con.execute( \
        "CREATE TABLE test.individuals AS SELECT i.* FROM source.individuals i JOIN test.families f ON i.histid = f.head_histid OR i.histid = f.spouse_histid;")

...
def child_snap():
    ...
    con.execute("""
        CREATE TEMP TABLE parent_map AS
    ...
    con.execute("""
        CREATE TEMP TABLE gedcom_children AS
    ...
    con.execute("""
        CREATE TEMP TABLE census_children AS
    ...
    con.execute("""
        CREATE TEMP TABLE matched_children AS
    ...
    if match_count > 0:
        ...
        con.execute("""
            ...
            SET vault_pointers = vault_pointers || '|' || (SELECT census_ptr FROM unique_matches m WHERE m.gedcom_child_id = golden_records.golden_id LIMIT 1),
                census_count = 1

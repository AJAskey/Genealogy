...
MATCH_DB_PATH = os.path.join(BASE_DATA_DIR, "DemographicMatchesTest.db")
...
def step_2_target_driven_extraction(con, logger):
    ...
    #     if 0 in (h_bpl, w_bpl): continue
    #     h_byr_int = int(h_byr)
    #     w_byr_int = int(w_byr)
    #     con.execute("INSERT INTO mem_targets VALUES (?, ?, ?, ?, ?, ?)", (h_byr_int - 5, h_byr_int + 5, h_bpl, w_byr_int - 5, w_byr_int + 5, w_bpl))
    #     targets_inserted += 1
    # logger.info(f"  -> Loaded {targets_inserted:,} fully-documented target couples into RAM filter.")

    logger.info("  -> JSON targets bypassed. Running targeted extraction for PA (42) in counties 270, 330, 350.")
    ...
    for year in range(1850, 1960, 10):
        ...
        con.execute(f"""
            CREATE TEMP TABLE local_fams AS SELECT family_id, head_histid, spouse_histid, countyicp FROM vault_{year}.families;
        ...
        # Extract all families where both Husband and Wife were born in PA and living in targeted counties
        con.execute(f"""
            ...
            WHERE h.sex = '1' AND s.sex = '2'
                AND f.countyicp IN (270, 330, 350)
                AND (CASE WHEN TRY_CAST(h.bpld AS INTEGER) >= 1000 THEN TRY_CAST(h.bpld AS INTEGER)//100 ELSE TRY_CAST(h.bpld AS INTEGER) END) = 42
                AND (CASE WHEN TRY_CAST(s.bpld AS INTEGER) >= 1000 THEN TRY_CAST(s.bpld AS INTEGER)//100 ELSE TRY_CAST(s.bpld AS INTEGER) END) = 42;
        """)
...
def step_6_consolidate_data(con, logger):
    ...
    con.execute("""
                ...
                SELECT clan_id, SUM(byr_int) AS lifetime_kfp
                FROM clan_kids
                GROUP BY clan_id;
                """)
...
def step_7_export_families_to_text(con, logger):
    """
    Dumps the constructed families directly to a text file (CSV)
    so they can be visually inspected outside of the database.
    """
    logger.info("\n=====================================================================")
    logger.info("STEP 7: EXPORTING FAMILIES TO TEXT FILE")
    logger.info("=====================================================================")

    output_txt = os.path.join(BASE_DATA_DIR, "Family_Dump_Test.csv").replace('\\', '/')

    con.execute(f"""
        COPY (
            SELECT 
                SUBSTRING(i.family_id, 1, 4) as census_year,
                i.family_id,
                CASE 
                    WHEN i.histid = f.head_histid THEN 'Head'
                    WHEN i.histid = f.spouse_histid THEN 'Spouse'
                    ELSE 'Child/Other'
                END as household_role,
                i.first_name,
                i.last_name,
                i.byr_int as birth_year,
                i.sex,
                i.bpl_int as birthplace,
                i.stateicp as residence_state,
                i.countyicp as residence_county
            FROM main.tm_individuals i
            JOIN main.tm_families f ON i.family_id = f.family_id
            ORDER BY i.family_id, household_role
        ) TO '{output_txt}' (HEADER, DELIMITER ',');
    """)

    logger.info(f"  -> Dumped all built families into: {output_txt}")


def main():
    logger = gen_logging.setup_logging('DemoTest')
    logger.info("=====================================================================")
    ...
    step_7_export_families_to_text(con, logger)

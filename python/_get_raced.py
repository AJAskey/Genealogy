...
"""
-----------------------------------
File: _get_raced.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for RACED codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for RACED) is detected
...
_json_path = r"E:/Claude/JSON/raced_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("RACED", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- RACED lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load RACED codes: {_e}")
...
def get_raced(code):
    """
    Returns the description for a RACED code.
...
if __name__ == "__main__":
    print(f"--- Testing RACED Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_raced(t)
            print(f"      get_raced({t!r:>8}) -> {result}")
    else:

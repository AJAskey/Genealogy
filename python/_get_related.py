...
"""
-----------------------------------
File: _get_related.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for RELATED codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for RELATED) is detected
...
_json_path = r"E:/Claude/JSON/related_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("RELATED", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- RELATED lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load RELATED codes: {_e}")


# ---------------------------------------------------------------------------
# 2. GETTER
# ---------------------------------------------------------------------------
def get_related(code):
    """
    Returns the description for a RELATED code.
...
if __name__ == "__main__":
    print(f"--- Testing RELATED Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_related(t)
            print(f"      get_related({t!r:>8}) -> {result}")
    else:

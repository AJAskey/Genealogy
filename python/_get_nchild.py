...
"""
-----------------------------------
File: _get_nchild.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for NCHILD codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for NCHILD) is detected
...
_json_path = r"E:/Claude/JSON/nchild_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("NCHILD", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- NCHILD lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load NCHILD codes: {_e}")
...
def get_nchild(code):
    """
    Returns the description for a NCHILD code.
...
if __name__ == "__main__":
    print(f"--- Testing NCHILD Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_nchild(t)
            print(f"      get_nchild({t!r:>8}) -> {result}")
    else:

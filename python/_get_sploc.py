...
"""
-----------------------------------
File: _get_sploc.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for SPLOC codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for SPLOC) is detected
...
_json_path = r"E:/Claude/JSON/sploc_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("SPLOC", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- SPLOC lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load SPLOC codes: {_e}")
...
def get_sploc(code):
    """
    Returns the description for a SPLOC code.
...
if __name__ == "__main__":
    print(f"--- Testing SPLOC Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_sploc(t)
            print(f"      get_sploc({t!r:>8}) -> {result}")
    else:

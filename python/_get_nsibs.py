...
"""
-----------------------------------
File: _get_nsibs.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for NSIBS codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for NSIBS) is detected
...
_json_path = r"E:/Claude/JSON/nsibs_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("NSIBS", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- NSIBS lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load NSIBS codes: {_e}")
...
def get_nsibs(code):
    """
    Returns the description for a NSIBS code.
...
if __name__ == "__main__":
    print(f"--- Testing NSIBS Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_nsibs(t)
            print(f"      get_nsibs({t!r:>8}) -> {result}")
    else:

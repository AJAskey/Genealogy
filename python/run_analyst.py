...
def run_analyst_pipeline(logger, mode="link", is_test=False, is_tracer=False):
    ...
    logger.info("Step 2/3: Extracting Base records (Scans 816M rows - THIS WILL TAKE 2 TO 6 MINUTES. DO NOT CANCEL!)...")
    ...
    if is_tracer:
        con.execute("""
            ...
            WHERE b_r.first IS NOT NULL AND REGEXP_MATCHES(b_r.first, '[a-zA-Z]')
            UNION ALL
            SELECT 
            ...
            WHERE u_d.first_name IS NOT NULL AND REGEXP_MATCHES(u_d.first_name, '[a-zA-Z]')
            UNION ALL
            SELECT 
            ...
            WHERE g_r.first_name IS NOT NULL AND REGEXP_MATCHES(g_r.first_name, '[a-zA-Z]')
        """)
    else:
        con.execute("""
        CREATE TABLE population_master AS
        ...
        SELECT 
            ...
            CAST(NULL AS VARCHAR) AS mother_pointer
        FROM birls.birls_records b_r
        WHERE b_r.last IS NOT NULL AND REGEXP_MATCHES(b_r.last, '[a-zA-Z]')
          AND b_r.first IS NOT NULL AND REGEXP_MATCHES(b_r.first, '[a-zA-Z]')
        UNION ALL
        SELECT 
            'UNIDEATH_' || u_d.record_id AS unique_id,
            u_d.first_name,
            ...
            CAST(NULL AS VARCHAR) AS mother_pointer
        FROM birls.universal_death_index u_d
        WHERE u_d.last_name IS NOT NULL AND REGEXP_MATCHES(u_d.last_name, '[a-zA-Z]')
          AND u_d.first_name IS NOT NULL AND REGEXP_MATCHES(u_d.first_name, '[a-zA-Z]')
        UNION ALL
        SELECT 
            'GED_' || g_r.gedcom_id AS unique_id,
            g_r.first_name,
            ...
            CAST(NULL AS VARCHAR) AS mother_pointer
        FROM gedcom.gedcom_records g_r
        WHERE g_r.last_name IS NOT NULL AND REGEXP_MATCHES(g_r.last_name, '[a-zA-Z]')
          AND g_r.first_name IS NOT NULL AND REGEXP_MATCHES(g_r.first_name, '[a-zA-Z]')
    """)

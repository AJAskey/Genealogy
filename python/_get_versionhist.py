...
"""
-----------------------------------
File: _get_versionhist.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for VERSIONHIST codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for VERSIONHIST) is detected
...
_json_path = r"E:/Claude/JSON/versionhist_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("VERSIONHIST", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- VERSIONHIST lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load VERSIONHIST codes: {_e}")


# ---------------------------------------------------------------------------
# 2. GETTER
# ---------------------------------------------------------------------------
def get_versionhist(code):
    """
    Returns the description for a VERSIONHIST code.
...
if __name__ == "__main__":
    print(f"--- Testing VERSIONHIST Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_versionhist(t)
            print(f"      get_versionhist({t!r:>8}) -> {result}")
    else:

...
"""
-----------------------------------
File: _get_eldch.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for ELDCH codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for ELDCH) is detected
...
_json_path = r"E:/Claude/JSON/eldch_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("ELDCH", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- ELDCH lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load ELDCH codes: {_e}")
...
def get_eldch(code):
    """
    Returns the description for a ELDCH code.
...
if __name__ == "__main__":
    print(f"--- Testing ELDCH Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_eldch(t)
            print(f"      get_eldch({t!r:>8}) -> {result}")
    else:

...
"""
-----------------------------------
File: _get_famsize.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for FAMSIZE codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for FAMSIZE) is detected
...
_json_path = r"E:/Claude/JSON/famsize_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("FAMSIZE", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- FAMSIZE lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load FAMSIZE codes: {_e}")


# ---------------------------------------------------------------------------
# 2. GETTER
# ---------------------------------------------------------------------------
def get_famsize(code):
    """
    Returns the description for a FAMSIZE code.
...
if __name__ == "__main__":
    print(f"--- Testing FAMSIZE Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_famsize(t)
            print(f"      get_famsize({t!r:>8}) -> {result}")
    else:

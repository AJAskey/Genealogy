...
def merge_gedcom(logger):
    ...
    con.execute("""
        CREATE TABLE IF NOT EXISTS clean.golden_records (
            ...
            state VARCHAR,
            death_date VARCHAR,
            census_years VARCHAR,
            record_count INTEGER,
            census_count INTEGER,
            death_record_count INTEGER,
            gedcom_count INTEGER,
            vault_pointers VARCHAR,
            father_pointer VARCHAR,
            mother_pointer VARCHAR,
            st_joes_patrilineal_id VARCHAR,
            st_joes_matrilineal_id VARCHAR
        );
        DELETE FROM clean.golden_records;
    """)
    ...
    query = """
        WITH squashed_census AS (
            ...
            WHERE LOWER(COALESCE(samp.namelast, base.namelast)) IN (SELECT DISTINCT LOWER(last_name) FROM gedcom.gedcom_records)
        ),
        matches AS (
            SELECT 
                g.gedcom_id,
            ...
            GROUP BY g.gedcom_id, g.first_name, g.last_name, g.birth_year, g.birth_place, g.death_date, g.father_gedcom_id, g.mother_gedcom_id
        )
        INSERT INTO clean.golden_records
        SELECT 
            'SJ_GED_' || m.gedcom_id AS golden_id,
            m.first_name,
            m.last_name,
            m.birth_year,
            m.birth_place,
            ...
            1 AS gedcom_count,
            'GED_' || m.gedcom_id || CASE WHEN m.census_cnt > 0 THEN '|' || m.census_ids ELSE '' END AS vault_pointers,
            CASE WHEN m.father_gedcom_id IS NOT NULL THEN 'GED_' || m.father_gedcom_id ELSE NULL END AS father_pointer,
            CASE WHEN m.mother_gedcom_id IS NOT NULL THEN 'GED_' || m.mother_gedcom_id ELSE NULL END AS mother_pointer,

...
"""
-----------------------------------
File: _get_serial.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for SERIAL codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for SERIAL) is detected
...
_json_path = r"E:/Claude/JSON/serial_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("SERIAL", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- SERIAL lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load SERIAL codes: {_e}")
...
def get_serial(code):
    """
    Returns the description for a SERIAL code.
...
if __name__ == "__main__":
    print(f"--- Testing SERIAL Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_serial(t)
            print(f"      get_serial({t!r:>8}) -> {result}")
    else:

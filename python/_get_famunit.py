...
"""
-----------------------------------
File: _get_famunit.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for FAMUNIT codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for FAMUNIT) is detected
...
_json_path = r"E:/Claude/JSON/famunit_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("FAMUNIT", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- FAMUNIT lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load FAMUNIT codes: {_e}")


# ---------------------------------------------------------------------------
# 2. GETTER
# ---------------------------------------------------------------------------
def get_famunit(code):
    """
    Returns the description for a FAMUNIT code.
...
if __name__ == "__main__":
    print(f"--- Testing FAMUNIT Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_famunit(t)
            print(f"      get_famunit({t!r:>8}) -> {result}")
    else:

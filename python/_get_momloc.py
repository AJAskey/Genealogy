...
"""
-----------------------------------
File: _get_momloc.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for MOMLOC codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for MOMLOC) is detected
...
_json_path = r"E:/Claude/JSON/momloc_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("MOMLOC", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- MOMLOC lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load MOMLOC codes: {_e}")
...
def get_momloc(code):
    """
    Returns the description for a MOMLOC code.
...
if __name__ == "__main__":
    print(f"--- Testing MOMLOC Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_momloc(t)
            print(f"      get_momloc({t!r:>8}) -> {result}")
    else:

...
"""
-----------------------------------
File: _get_yngch.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for YNGCH codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for YNGCH) is detected
...
_json_path = r"E:/Claude/JSON/yngch_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("YNGCH", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- YNGCH lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load YNGCH codes: {_e}")
...
def get_yngch(code):
    """
    Returns the description for a YNGCH code.
...
if __name__ == "__main__":
    print(f"--- Testing YNGCH Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_yngch(t)
            print(f"      get_yngch({t!r:>8}) -> {result}")
    else:

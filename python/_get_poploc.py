...
"""
-----------------------------------
File: _get_poploc.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for POPLOC codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for POPLOC) is detected
...
_json_path = r"E:/Claude/JSON/poploc_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("POPLOC", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- POPLOC lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load POPLOC codes: {_e}")
...
def get_poploc(code):
    """
    Returns the description for a POPLOC code.
...
if __name__ == "__main__":
    print(f"--- Testing POPLOC Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_poploc(t)
            print(f"      get_poploc({t!r:>8}) -> {result}")
    else:

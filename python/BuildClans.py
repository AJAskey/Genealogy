...
def build_clans(logger):
    ...
    with sqlite3.connect(MATCH_DB) as conn:
        ...
        logger.info(f"  -> Successfully wove {len(clans):,} distinct multi-generational Clans in {time.time() - start_time:.2f} seconds.")
        ...
        logger.info(f"  -> Saved {len(clan_mappings):,} family-to-clan mappings in {time.time() - start_time:.2f} seconds.")

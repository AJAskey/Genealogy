...
"""
-----------------------------------
File: _get_histid.py  (auto-generated by IpumsCodebookParser.py)

Summary: Data Access Layer for HISTID codes.
         Accepts any numeric representation of the code:
         integer or string, with or without leading zeros.
         Key width (4 digits for HISTID) is detected
...
_json_path = r"E:/Claude/JSON/histid_codes.json"

try:
    with open(_json_path, 'r', encoding='utf-8') as _f:
        _raw = json.load(_f)
    _map = _raw.get("HISTID", _raw)
    _width = len(next(iter(_map))) if _map else 1
except FileNotFoundError:
    print(f"WARNING: {_json_path} not found -- HISTID lookups will return Unknown")
except Exception as _e:
    print(f"WARNING: could not load HISTID codes: {_e}")
...
def get_histid(code):
    """
    Returns the description for a HISTID code.
...
if __name__ == "__main__":
    print(f"--- Testing HISTID Getter (key width = {_width}) ---")
    ...
    if _map:
        ...
        for t in test_inputs:
            result = get_histid(t)
            print(f"      get_histid({t!r:>8}) -> {result}")
    else:
